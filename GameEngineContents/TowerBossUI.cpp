...
void TowerBossUI::Start()
{
...

	{
		...
		RendererButton1->GetTransform().SetLocalPosition({ -180.f,140.f, -250.f });
...

	{
		...
		RendererButton2->SetTexture("LastLord.png");
		//RendererButton2->ScaleToTexture();
		RendererButton2->GetTransform().SetLocalScale({ 1000, 200, 1 });
		RendererButton2->GetTransform().SetLocalPosition({ 0.f,40.f, -250.f });
...
void TowerBossUI::Update(float _DeltaTime)
{
